...
function simBWF.query()
    -- dummy function, normally implemented via the simBWF plugin
end
...
function simBWF.getHoneycombPalletPoints(rows,rowStep,cols,colStep,layers,layerStep,firstRowIsOdd,pointsAreCentered,optionalGlobalOffset)
    ...
    for k=1,layers,1 do
        for j=1,cols,1 do
            local r=rowSize[1+(j-1&1)]
            for i=1,r,1 do
                local rowOff=rowOffs[1+(j-1&1)]
                local decItem=simBWF.getOneRawPalletItem()
...
function simBWF.getAllJobNames()
    ...
    local currentJob='default' --=sim.getStringParameter(sim.stringparam_job)
...
function simBWF.handleJobConsistency_generic(removeJobsExceptCurrent)
    ...
    model.currentJob='default' --=sim.getStringParameter(sim.stringparam_job)
...
function simBWF.createNewJob_generic()
    ...
    local newJobName='default' --=sim.getStringParameter(sim.stringparam_job)
    local data=model.readInfo()
